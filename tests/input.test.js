...
test("地面 W/S 控制撑杖和刹车，不再触发翻转，摇杆提供模拟量", () => {
  assert.equal(input(["KeyW"], [], { grounded: true }).tuck, 1);
  assert.equal(input(["KeyS"], [], { grounded: true }).brake, 1);
  assert.equal(input(["KeyS"], [], { grounded: true }).flip, 0);
  assert.equal(
    input(["ArrowUp"], [], { grounded: true, heldMilliseconds: 300 }).jump,
    false,
  );
  const analog = input([], [], {
    grounded: true,
    joystick: { x: 0.42, y: 0.65 },
  });
  assert.equal(analog.steer, 0.42);
  assert.equal(analog.brake, 0.65);
});
