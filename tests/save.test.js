...
test('新存档提供独立默认对象', () => {
  assert.ok(api?.readSave && api?.writeSave && api?.bankRun && api?.canUseSkin, '存档模块应导出约定 API');
  const save = api.readSave(storageFor());
  assert.deepEqual(save, {
    bestDistance: 0, bestScore: 0, totalCoins: 0, runs: 0,
    skin: 'orange', sound: true, quality: 'high',
  });
...
test('损坏 JSON、非对象和禁用存储安全返回默认存档', () => {
  ...
  const deniedStorage = { getItem() { throw new Error('SecurityError'); } };
...
test('异常数值、设置和额外字段被规范化', () => {
  assert.ok(api);
  const save = api.readSave(storageFor(JSON.stringify({
    bestDistance: -4,
    bestScore: '999',
    totalCoins: 123.9,
    runs: null,
  ...
  const extreme = api.readSave(storageFor('{"bestDistance":1e400,"totalCoins":1e100,"runs":-1}'));
...
test('加载保留合法设置并拒绝使用未解锁皮肤', () => {
  assert.ok(api);
  const unlocked = api.readSave(storageFor(JSON.stringify({ totalCoins: 100, skin: 'blue', sound: false, quality: 'low' })));
  assert.equal(unlocked.skin, 'blue');
  assert.equal(unlocked.sound, false);
  assert.equal(unlocked.quality, 'low');
  const locked = api.readSave(storageFor(JSON.stringify({ totalCoins: 299, skin: 'violet' })));
...
test('正常写入使用约定键且能完整往返读取', () => {
  ...
  const save = { ...api.DEFAULT_SAVE, totalCoins: 300, skin: 'violet', sound: false, bestScore: 900 };
...
test('写入失败返回 false，写入成功前规范异常输入', () => {
  assert.ok(api);
  assert.equal(api.writeSave(api.DEFAULT_SAVE, { setItem() { throw new Error('QuotaExceededError'); } }), false);
  assert.equal(api.writeSave(api.DEFAULT_SAVE, null), false);
  const storage = storageFor();
  assert.equal(api.writeSave({ totalCoins: -9, skin: 'blue', bestScore: Number.NaN }, storage), true);
  assert.deepEqual(api.readSave(storage), api.DEFAULT_SAVE);
});

test('结算累计金币和场次、更新整数纪录并保留原对象', () => {
  assert.ok(api);
  const original = { ...api.DEFAULT_SAVE, totalCoins: 95, runs: 2, bestDistance: 600, bestScore: 1200 };
  const next = api.bankRun(original, { coins: 12.9, distance: 850.8, score: 1000.2 });
  assert.deepEqual(next, { ...original, totalCoins: 107, runs: 3, bestDistance: 850 });
...
test('结算异常局数据不污染账户且加法不超过安全整数', () => {
  assert.ok(api);
  const save = api.bankRun({ ...api.DEFAULT_SAVE, totalCoins: Number.MAX_SAFE_INTEGER, runs: Number.MAX_SAFE_INTEGER }, {
    coins: 10, distance: Infinity, score: -100,
  });
