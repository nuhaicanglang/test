...
test('同一实体编号换成另一障碍时替换模型并归还原类型池', () => {
  ...
  world.updateEntities({ distance: 0, entities: [{ id: 'snow-0', type: 'tree', x: 0, d: 40 }] });
  const original = world.entityMeshes.get('snow-0');
  world.updateEntities({ distance: 0, entities: [{ id: 'snow-0', type: 'rock', x: 3, d: 40 }] });
...
test('不同种子连续重开后所有实体模型都与碰撞类型一致', () => {
  ...
  for (const seed of [1, 2, 12, 125, 42, 20260905]) {
    ...
    for (const entity of game.state.entities) {
      ...
      if (entity.d > 250) continue;
      assert.ok(mesh);
      assert.equal(mesh.userData.createdType, entity.type, `种子 ${seed} 的 ${entity.id} 模型应匹配规则`);
...
test('已收集对象移出场景，同类新实体复用原模型', () => {
  ...
  world.updateEntities({ distance: 0, entities: [{ id: 'coin-after', type: 'coin', x: -3, d: 50 }] });
