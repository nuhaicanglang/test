export class GameAudio {
  ...
  play(type, turns = 1) {
    if (type === "avalanche-warning") {
      [0, 0.45, 0.9].forEach((delay) =>
        this.tone(115, 0.32, "triangle", 0.3, delay),
      );
    }
    if (type === "coin") {
      this.tone(1046, 0.09, "sine", 0.22);
      this.tone(1568, 0.15, "sine", 0.14, 0.04);
    }
    if (type === "jump" || type === "ramp" || type === "terrain") {
      this.tone(330, 0.12, "triangle");
      this.tone(495, 0.15, "sine", 0.13, 0.07);
    }
    if (type === "airturn") {
    ...
    if (type === "flip" || type === "powerup")
      [523, 659, 784, 1046].forEach((note, i) =>
        this.tone(note, 0.18, "triangle", 0.16, i * 0.065),
      );
    if (type === "crash") {
      this.tone(70, 0.3, "sawtooth", 0.17);
      this.tone(49, 0.4, "triangle", 0.2);
    }
    if (type === "start")
      [392, 523, 784].forEach((note, i) =>
        this.tone(note, 0.2, "sine", 0.2, i * 0.09),
      );
    if (type === "gameover")
      [392, 330, 262].forEach((note, i) =>
        this.tone(note, 0.3, "triangle", 0.14, i * 0.15),
      );
  ...
  update(state) {
    ...
    const volume =
      state.phase === "playing"
        ? (0.07 + state.speed * 0.004) * (state.y > 0.3 ? 0.3 : 1)
        : 0;
