import { normalizeQuality, defaultQuality } from "./quality.js";
...
export function canUseSkin(id, totalCoins) {
  return (
    Object.hasOwn(SKIN_COSTS, id) && safeInteger(totalCoins) >= SKIN_COSTS[id]
  );
}
...
function normalizeSave(value) {
  ...
  return {
    ...
    quality: normalizeQuality(input.quality, defaultQuality()),
    motion: typeof input.motion === "boolean" ? input.motion : true,
    startTime: ["dawn", "noon", "sunset", "night"].includes(input.startTime)
      ? input.startTime
      : "dawn",
    openBestDistance: safeInteger(input.openBestDistance),
    openBestScore: safeInteger(input.openBestScore),
  };
}
...
export function readSave(storage) {
  try {
    ...
    return raw
      ...
      : { ...DEFAULT_SAVE, quality: defaultQuality() };
  } catch {
    return { ...DEFAULT_SAVE, quality: defaultQuality() };
  }
}
...
export function bankRun(save, state) {
  ...
  return {
    ...
    bestDistance:
      result.mode === "freeride"
        ? current.bestDistance
        : Math.max(current.bestDistance, safeInteger(result.distance)),
    bestScore:
      result.mode === "freeride"
        ? current.bestScore
        : Math.max(current.bestScore, safeInteger(result.score)),
    openBestDistance:
      result.mode === "freeride"
        ? Math.max(current.openBestDistance, safeInteger(result.distance))
        : current.openBestDistance,
    openBestScore:
      result.mode === "freeride"
        ? Math.max(current.openBestScore, safeInteger(result.score))
        : current.openBestScore,
  };
}
