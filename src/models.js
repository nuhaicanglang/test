...
import {
  ...
  createDetailedTree,
  createDetailedRock,
  createDetailedMountain,
} from "./detail-models.js";
...
function material(name, color, extras = {}) {
  if (!materialCache.has(name)) {
    const result = makeSurface(name, color, {
      roughness: 0.78,
      metalness: 0,
      flatShading: false,
      ...extras,
...
const palette = {
  ...
  blue: material("blue", 0x357188),
  mint: material("mint", 0x547375),
  coral: material("coral", 0xe05e38),
  yellow: material("yellow", 0xddb16a),
...
const sphereGeometry = () =>
  geometry("sphere", () => new THREE.SphereGeometry(1, 12, 8));
const icoGeometry = () =>
  geometry("ico", () => new THREE.IcosahedronGeometry(1, 0));
const dodecaGeometry = () =>
  geometry("dodeca", () => new THREE.DodecahedronGeometry(1, 0));
const cylinderGeometry = () =>
  geometry("cylinder", () => new THREE.CylinderGeometry(1, 1, 1, 10));
const coneGeometry = () =>
  geometry("cone", () => new THREE.ConeGeometry(1, 1, 9));
...
function torus(
  ...
  return part(
    ...
    geometry(
      ...
      () => new THREE.TorusGeometry(radius, thickness, 6, 24),
    ),
    surface,
    position,
    [1, 1, 1],
...
function ski(parent, x, surface) {
  ...
  part(
    root,
    extrudedShape("ski-profile", shape, 0.055, 0.012),
    surface,
    [0, 0.0395, 0],
    [1, 1, 1],
    [-Math.PI / 2, 0, 0],
...
export const createSkier = createDetailedSkier;

export const createTree = createDetailedTree;

export const createRock = createDetailedRock;
...
export function createLog() {
  ...
  for (let i = 0; i < 7; i += 1) {
    ...
    beam(
      root,
      palette.wood,
      [-1.96, 0.42 + Math.sin(angle) * 0.39, Math.cos(angle) * 0.39],
      [1.96, 0.42 + Math.sin(angle) * 0.39, Math.cos(angle) * 0.39],
      0.024,
    );
  }
...
export function createRamp() {
  ...
  const wedge = geometry("ramp-wedge", () => {
    ...
    result.setAttribute(
      ...
      new THREE.Float32BufferAttribute(
        [
          -2, 0.02, -3, 2, 0.02, -3, -2, 0.02, 3, 2, 0.02, 3, -2, 1.5, -3, 2,
          1.5, -3, -2, 0.1, 3, 2, 0.1, 3,
        ],
        3,
      ),
    );
    result.setIndex([
      0, 4, 5, 0, 5, 1, 2, 3, 7, 2, 7, 6, 0, 2, 6, 0, 6, 4, 1, 5, 7, 1, 7, 3, 4,
      6, 7, 4, 7, 5, 0, 1, 3, 0, 3, 2,
    ]);
  ...
  part(
    root,
    extrudedShape("ramp-arrow", arrowShape, 0.01, 0),
    palette.yellow,
    [0, 0.93, 0],
    [1, 1, 1],
    [-Math.PI / 2 + angle, 0, 0],
...
function createPenguin() {
  ...
  for (const sign of [-1, 1]) {
    sphere(
      root,
      palette.snow,
      [sign * 0.118, 1.12, -0.238],
      [0.155, 0.19, 0.092],
    );
    sphere(
      root,
      palette.dark,
      [sign * 0.117, 1.183, -0.327],
      [0.038, 0.047, 0.026],
    );
    sphere(
      root,
      palette.snow,
      [sign * 0.11 - 0.008, 1.197, -0.35],
      [0.009, 0.011, 0.007],
    );
    const wing = sphere(
      root,
      palette.navy,
      [sign * 0.447, 0.67, 0.005],
      [0.105, 0.335, 0.175],
    );
...
export function createPowerup(type) {
  ...
  const cyanGlow = material("shield-glow", 0x75e6ff, {
    emissive: 0x2199c1,
    emissiveIntensity: 0.7,
    metalness: 0.25,
    roughness: 0.23,
  });
  const redGlow = material("magnet-glow", 0xff866b, {
    emissive: 0xb52d35,
  ...
  if (type === "shield") {
    ...
    part(
      root,
      extrudedShape("shield-icon", shield, 0.13, 0.035),
      palette.navy,
      [0, 0, -0.092],
      [0.76, 0.76, 0.25],
    );
    ...
    for (const sign of [-1, 1])
      box(root, palette.snow, [sign * 0.365, 0.38, 0], [0.284, 0.19, 0.21]);
...
export function createCabin() {
  ...
  for (let i = 0; i < 9; i += 1) {
    ...
    for (const sign of [-1, 1]) {
      part(
        root,
        cylinderGeometry(),
        i % 2 ? palette.wood : palette.woodLight,
        [0, y, sign * 2.015],
        [0.177, 6.22, 0.177],
        [0, 0, Math.PI / 2],
      );
      part(
        root,
        cylinderGeometry(),
        i % 2 ? palette.woodLight : palette.wood,
        [sign * 3.015, y, 0],
        [0.175, 4.22, 0.175],
        [Math.PI / 2, 0, 0],
      );
  ...
  gable.closePath();
  for (const sign of [-1, 1])
    part(root, extrudedShape("cabin-gable", gable, 0.15, 0), palette.wood, [
      0,
      3,
      sign * 2.01,
    ]);
  ...
  for (const sign of [-1, 1]) {
    ...
    box(
      ...
      palette.snow,
      [sign * 1.8, 4.045, 0],
      [Math.hypot(3.6, 1.8) + 0.06, 0.21, 5.19],
      [0, 0, -sign * angle],
    );
    beam(
      root,
  ...
  const warmGlass = material("window-glow", 0xffd98a, {
    emissive: 0xffad45,
...
export const createMountain = createDetailedMountain;
...
export function createGate(label = "SUMMIT RUN") {
        ...
        if ((i + j) % 2 === 0) {
          for (const side of [-1, 1])
            box(
              root,
              palette.snow,
              [sign * (6.5 + i * 0.64), 5.74 + j * 0.58, side * 0.073],
              [0.64, 0.58, 0.016],
            );
        }
      }
  ...
  part(
    root,
    geometry("gate-label-plane", () => new THREE.PlaneGeometry(11.5, 1.48)),
    gateLabel(String(label)),
    [0, 6.05, -0.104],
    [1, 1, 1],
    [0, Math.PI, 0],
  );
...
export function disposeObject(object) {
  ...
  const release = (resource) => {
    if (
      !resource ||
      resource.userData?.sharedModelResource ||
